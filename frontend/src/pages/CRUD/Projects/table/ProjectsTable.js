...
const ProjectsTable = () => {
  ...
  const columns = [

      {
        ...
        flex: 0.6,
        maxWidth: 80,
        renderCell: (params) => <Actions classes={classes} entity="projects" openModal={openModal} {...params} />,
      }
  ];

  return (
    <div>
      <Widget title={<h4>{humanize('Projects')}</h4>} disableWidgetMenu>
        <Box className={classes.actions}>
          <Link to="/admin/projects/new" className={classes.element}>
          ...
          <Button type='button' variant='contained' onClick={getProjectsCSV} className={classes.element}>
            Export CSV
          </Button>
        </Box>
        ...
        <div style={{minHeight: 500, width: "100%", paddingTop: 20, paddingBottom: 20}}>
          <DataGrid
            ...
            disableColumnMenu
            loading={loading}
            onRowClick={(e) => {history.push(`/admin/projects/${e.id}/edit`)}}
            autoHeight
          />
        </div>
