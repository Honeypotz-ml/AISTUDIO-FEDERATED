...
async function selectList(query, limit) {
  ...
  const response = await axios.get(`/algorithms/autocomplete`, { params });
  return response.data;
}

const AlgorithmsSelectItem = (props) => {
...
const select = (state) => ({
  hasPermissionToCreate: state.algorithms.hasPermissionToCreate,
});

export default connect(select)(AlgorithmsSelectItem);
