...
import * as dataFormat from 'pages/CRUD/Deployment/table/DeploymentDataFormatters';

import actions from 'actions/deployment/deploymentListActions';
...
const DeploymentTable = () => {
  ...
  const count = useSelector((store) => store.deployment.list.count);
  const modalOpen = useSelector((store) => store.deployment.list.modalOpen);
  const rows = useSelector((store) => store.deployment.list.rows);
  const idToDelete = useSelector((store) => store.deployment.list.idToDelete);
  ...
  const getDeploymentCSV = async () => {
    const response = await axios({url: '/deployment?filetype=csv', method: 'GET',responseType: 'blob'});
    const type = response.headers['content-type']
    const blob = new Blob([response.data], { type: type, encoding: 'UTF-8' })
    const link = document.createElement('a')
    link.href = window.URL.createObjectURL(blob)
    link.download = 'deploymentCSV.csv'
    link.click()
  };
  ...
  const columns = [

      {
        ...
        flex: 0.6,
        maxWidth: 80,
        renderCell: (params) => <Actions classes={classes} entity="deployment" openModal={openModal} {...params} />,
      }
  ];

  return (
    <div>
      <Widget title={<h4>{humanize('Deployment')}</h4>} disableWidgetMenu>
        <Box className={classes.actions}>
          <Link to="/admin/deployment/new" className={classes.element}>
          ...
          <Button type='button' variant='contained' onClick={getDeploymentCSV} className={classes.element}>
            Export CSV
          </Button>
        </Box>
        ...
        <div style={{minHeight: 500, width: "100%", paddingTop: 20, paddingBottom: 20}}>
          <DataGrid
            ...
            disableColumnMenu
            loading={loading}
            onRowClick={(e) => {history.push(`/admin/deployment/${e.id}/edit`)}}
            autoHeight
          />
        </div>
          ...
          <LinkMaterial
            ...
            href={
              process.env.NODE_ENV === 'production'
                ? window.location.origin + '/api-docs/#/Deployment'
                : 'http://localhost:8080/api-docs/#/Deployment'
            }
          >
