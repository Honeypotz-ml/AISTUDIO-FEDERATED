...
const PaymentsTable = () => {
  ...
  const columns = [

      {
        ...
        flex: 0.6,
        maxWidth: 80,
        renderCell: (params) => <Actions classes={classes} entity="payments" openModal={openModal} {...params} />,
      }
  ];

  return (
    <div>
      <Widget title={<h4>{humanize('Payments')}</h4>} disableWidgetMenu>
        <Box className={classes.actions}>
          <Link to="/admin/payments/new" className={classes.element}>
          ...
          <Button type='button' variant='contained' onClick={getPaymentsCSV} className={classes.element}>
            Export CSV
          </Button>
        </Box>
        ...
        <div style={{minHeight: 500, width: "100%", paddingTop: 20, paddingBottom: 20}}>
          <DataGrid
            ...
            disableColumnMenu
            loading={loading}
            onRowClick={(e) => {history.push(`/admin/payments/${e.id}/edit`)}}
            autoHeight
          />
        </div>
