...
import * as dataFormat from 'pages/CRUD/Teams/table/TeamsDataFormatters';

import actions from 'actions/teams/teamsListActions';
...
const TeamsTable = () => {
  ...
  const [filters, setFilters] = React.useState([
    {label: 'TeamName', title: 'team_name'},{label: 'Description', title: 'description'},{label: 'Company', title: 'company'},
  ...
  const count = useSelector((store) => store.teams.list.count);
  const modalOpen = useSelector((store) => store.teams.list.modalOpen);
  const rows = useSelector((store) => store.teams.list.rows);
  const idToDelete = useSelector((store) => store.teams.list.idToDelete);
  ...
  const columns = [
...

      {
        ...
        flex: 0.6,
        maxWidth: 80,
        renderCell: (params) => <Actions classes={classes} entity="teams" openModal={openModal} {...params} />,
      }
  ];

  return (
    <div>
      <Widget title={<h4>{humanize('Teams')}</h4>} disableWidgetMenu>
        <Box className={classes.actions}>
          <Link to="/admin/teams/new" className={classes.element}>
          ...
          <Button type='button' variant='contained' onClick={getTeamsCSV} className={classes.element}>
            Export CSV
          </Button>
        </Box>
        ...
        <div style={{minHeight: 500, width: "100%", paddingTop: 20, paddingBottom: 20}}>
          <DataGrid
            ...
            disableColumnMenu
            loading={loading}
            onRowClick={(e) => {history.push(`/admin/teams/${e.id}/edit`)}}
            autoHeight
          />
        </div>
