...
import * as dataFormat from 'pages/CRUD/Federation/table/FederationDataFormatters';

import actions from 'actions/federation/federationListActions';
...
const FederationTable = () => {
  ...
  const count = useSelector((store) => store.federation.list.count);
  const modalOpen = useSelector((store) => store.federation.list.modalOpen);
  const rows = useSelector((store) => store.federation.list.rows);
  const idToDelete = useSelector((store) => store.federation.list.idToDelete);
  ...
  const getFederationCSV = async () => {
    const response = await axios({url: '/federation?filetype=csv', method: 'GET',responseType: 'blob'});
    const type = response.headers['content-type']
    const blob = new Blob([response.data], { type: type, encoding: 'UTF-8' })
    const link = document.createElement('a')
    link.href = window.URL.createObjectURL(blob)
    link.download = 'federationCSV.csv'
    link.click()
  };
  ...
  const columns = [

      {
        ...
        flex: 0.6,
        maxWidth: 80,
        renderCell: (params) => <Actions classes={classes} entity="federation" openModal={openModal} {...params} />,
      }
  ];

  return (
    <div>
      <Widget title={<h4>{humanize('Federation')}</h4>} disableWidgetMenu>
        <Box className={classes.actions}>
          <Link to="/admin/federation/new" className={classes.element}>
          ...
          <Button type='button' variant='contained' onClick={getFederationCSV} className={classes.element}>
            Export CSV
          </Button>
        </Box>
        ...
        <div style={{minHeight: 500, width: "100%", paddingTop: 20, paddingBottom: 20}}>
          <DataGrid
            ...
            disableColumnMenu
            loading={loading}
            onRowClick={(e) => {history.push(`/admin/federation/${e.id}/edit`)}}
            autoHeight
          />
        </div>
          ...
          <LinkMaterial
            ...
            href={
              process.env.NODE_ENV === 'production'
                ? window.location.origin + '/api-docs/#/Federation'
                : 'http://localhost:8080/api-docs/#/Federation'
            }
          >
            API documentation for federation
          </LinkMaterial>
        </div>
      </Widget>
