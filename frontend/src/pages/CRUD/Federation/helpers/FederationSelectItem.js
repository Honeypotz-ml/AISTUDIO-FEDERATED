...
async function selectList(query, limit) {
  ...
  const response = await axios.get(`/federation/autocomplete`, { params });
  return response.data;
}

const FederationSelectItem = (props) => {
...
const select = (state) => ({
  hasPermissionToCreate: state.federation.hasPermissionToCreate,
});

export default connect(select)(FederationSelectItem);
