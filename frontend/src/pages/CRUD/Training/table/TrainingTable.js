...
import * as dataFormat from 'pages/CRUD/Training/table/TrainingDataFormatters';

import actions from 'actions/training/trainingListActions';
...
const TrainingTable = () => {
  ...
  const count = useSelector((store) => store.training.list.count);
  const modalOpen = useSelector((store) => store.training.list.modalOpen);
  const rows = useSelector((store) => store.training.list.rows);
  const idToDelete = useSelector((store) => store.training.list.idToDelete);
  ...
  const getTrainingCSV = async () => {
    const response = await axios({url: '/training?filetype=csv', method: 'GET',responseType: 'blob'});
    const type = response.headers['content-type']
    const blob = new Blob([response.data], { type: type, encoding: 'UTF-8' })
    const link = document.createElement('a')
    link.href = window.URL.createObjectURL(blob)
    link.download = 'trainingCSV.csv'
    link.click()
  };
  ...
  const columns = [

      {
        ...
        flex: 0.6,
        maxWidth: 80,
        renderCell: (params) => <Actions classes={classes} entity="training" openModal={openModal} {...params} />,
      }
  ];

  return (
    <div>
      <Widget title={<h4>{humanize('Training')}</h4>} disableWidgetMenu>
        <Box className={classes.actions}>
          <Link to="/admin/training/new" className={classes.element}>
          ...
          <Button type='button' variant='contained' onClick={getTrainingCSV} className={classes.element}>
            Export CSV
          </Button>
        </Box>
        ...
        <div style={{minHeight: 500, width: "100%", paddingTop: 20, paddingBottom: 20}}>
          <DataGrid
            ...
            disableColumnMenu
            loading={loading}
            onRowClick={(e) => {history.push(`/admin/training/${e.id}/edit`)}}
            autoHeight
          />
        </div>
          ...
          <LinkMaterial
            ...
            href={
              process.env.NODE_ENV === 'production'
                ? window.location.origin + '/api-docs/#/Training'
                : 'http://localhost:8080/api-docs/#/Training'
            }
          >
            API documentation for training
          </LinkMaterial>
        </div>
      </Widget>
